...
class BigQueryService {
  ...
  Future<List<Map<String, dynamic>>> executeQuery(
    ...
    try {
      ...
      if (response.jobComplete == false) {
        ...
        log('Warning: Query job did not complete immediately. Results might be partial or delayed.');
        // You might want to implement polling here similar to the upload job
      }

      if (response.errors != null && response.errors!.isNotEmpty) {
        final errorMessage = response.errors!.map((e) => '${e.reason}: ${e.message}').join(', ');
            ...
            if (i < row.f!.length) {
              ...
              if (value != null) {
                  if (field.type == 'INTEGER' || field.type == 'INT64') {
                    rowData[field.name!] = int.tryParse(value.toString()) ?? value;
                  } else if (field.type == 'FLOAT' || field.type == 'FLOAT64' || field.type == 'NUMERIC' || field.type == 'BIGNUMERIC') {
                    rowData[field.name!] = double.tryParse(value.toString()) ?? value;
                  } else if (field.type == 'BOOLEAN' || field.type == 'BOOL') {
                    rowData[field.name!] = value.toString().toLowerCase() == 'true';
                  } else {
                      rowData[field.name!] = value; // Keep as string or original type
                  }
              } else {
                  rowData[field.name!] = null;
              }
            } else {
               rowData[field.name!] = null; // Handle cases where row has fewer fields than schema (shouldn't happen in valid response)
               log('Warning: Row ${response.rows!.indexOf(row)} has fewer fields than schema expected.');
            }
          }
          rows.add(rowData);
        }
      ...
      if (e is DetailedApiRequestError) {
         throw Exception('Failed to execute query: DetailedApiRequestError(status: ${e.status}, message: ${e.message})');
      } else {
  ...
  Future<List<String>> getDatasets() async {
    ...
    try {
      ...
      final datasetIds = datasets.datasets?.map((d) => d.datasetReference?.datasetId ?? '').where((id) => id.isNotEmpty).toList() ?? [];
  ...
  Future<List<String>> getTables(String datasetId) async {
    ...
    try {
      ...
      final tableIds = tables.tables?.map((t) => t.tableReference?.tableId ?? '').where((id) => id.isNotEmpty).toList() ?? [];
  ...
  Future<String> getTableSchema(String datasetId, String tableId) async {
    ...
    try {
      ...
      final table = await _bigQueryApi.tables.get(projectId, datasetId, tableId);
  ...
  Future<void> uploadCsvToTable(
    ...
    String createDisposition = 'CREATE_IF_NEEDED', // Default: create table if not exists
    ...
    try {
      ...
      final jobConfigLoad = JobConfigurationLoad()
      ...
      final job = Job()
        ..configuration = (JobConfiguration()..load = jobConfigLoad);

      final media = Media(Stream<List<int>>.value(csvBytes.toList()), csvBytes.length);

      log('Inserting BigQuery load job...');
      final insertResponse = await _bigQueryApi.jobs.insert(job, projectId, uploadMedia: media);

      final jobId = insertResponse.jobReference?.jobId;
      final location = insertResponse.jobReference?.location; // *** Get the job location ***

      if (jobId == null) {
        log('Error: Job insertion did not return a jobId.');
        throw Exception('Failed to start BigQuery load job: No Job ID returned.');
      }
      if (location == null) {
        // Although unlikely, handle missing location just in case.
        // The job might still succeed, but polling will fail.
        log('Warning: Job insertion did not return a location. Polling might fail.');
         throw Exception('Failed to start BigQuery load job: No location returned.');
      }

      log('BigQuery load job started: $jobId in location: $location. Polling for completion...');
      ...
      while (stopwatch.elapsed < timeout) {
        ...
        final jobStatus = await _bigQueryApi.jobs.get(projectId, jobId, location: location);
        final state = jobStatus.status?.state;
        log('Polling job $jobId (location: $location): State = $state, Elapsed = ${stopwatch.elapsed}');
        ...
        if (state == 'DONE') {
          ...
          if (errorResult != null) {
            ...
            String allErrors = jobStatus.status?.errors?.map((e) => '${e.reason}: ${e.message} at ${e.location}').join('') ?? '';
            final errorMessage = '${errorResult.reason}: ${errorResult.message}';
            log('BigQuery load job $jobId failed: $errorMessage Details: $allErrors', error: jobStatus.status?.errors);
      ...
      throw Exception('BigQuery load job timed out after ${timeout.inSeconds} seconds.');

    } catch (e) {
      log('Failed to upload CSV: $e', error: e);
       if (e is DetailedApiRequestError) {
         // Provide more context for API errors
         throw Exception('Failed to upload CSV: API Error (status: ${e.status}, message: ${e.message})');
      } else {
